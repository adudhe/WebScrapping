...
CONFIG = {
    'Solarkal': {
        ...
        'hq_offices': {'div': 'offices-list', 'class': 'office-location'},
    ...
    'H2Scan': {
        'description': {'tag': 'div', 'class': 'elementor-heading-title'},
        'hq_offices': {'tag': 'div', 'class': 'elementor-widget-container'},  # Adjusted for H2Scan
        'clients': {'div': 'elementor-image-carousel', 'tag': 'img', 'attribute': 'src'},
        'news': {}  # Add configuration if needed
    },
    # Add configurations for other websites similarly
}
...
def scrape_company_info(base_url, blog_url, company_name):
    ...
    data = {
        'Description': '',
        'HQ and Offices': '',  # Initialize as empty string
        'Clients': [],
        'News': []
    }
        ...
        if clients_config:
            ...
            if clients_section:
                ...
                for img in img_tags:
                    ...
                    if img_src:
                        filename_with_extension = img_src.split('/')[-1]
                        filename = filename_with_extension.split('.')[0]
                        parts = filename.split('_')  # Split by underscores
                        client_name = parts[-1] if len(parts) > 1 else filename
...
if __name__ == '__main__':
    ...
    # Save data to Excel
    if all_data:
        save_to_excel(all_data)  # Save all data in one file
